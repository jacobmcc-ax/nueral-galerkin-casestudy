...
class TestSparseSampling:
    ...
    def setup_method(self):
        ...
        self.efficiency_threshold = 2.0  # At least 2x speedup expected
    ...
    def test_sparse_galerkin_projection_efficiency(self):
        ...
        try:
            ...
            # Time sparse Galerkin projection
            start_time = time.time()
            ...
            efficiency_ratio = full_time / sparse_time if sparse_time > 0 else 0

            log_mathematical_result(
                "Sparse Galerkin projection efficiency",
                "PASS" if efficiency_ratio >= self.efficiency_threshold else "FAIL",
                f"Speedup: {efficiency_ratio:.2f}x",
                f"Expected speedup >= {self.efficiency_threshold}x"
            )

            assert efficiency_ratio >= self.efficiency_threshold, f"Efficiency ratio {efficiency_ratio:.2f} < threshold {self.efficiency_threshold}"
    ...
    def test_sparse_accuracy_preservation(self):
        ...
        try:
            ...
            accuracy_preserved = accuracy_ratio < 10.0  # Allow 10x degradation
            ...
            assert converged_sparse, "Sparse method should converge"
            assert accuracy_preserved, f"Accuracy degradation {accuracy_ratio:.2f} > 10x threshold"
